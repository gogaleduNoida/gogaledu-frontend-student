...
const CourseDetailPage = ({ params }) => {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      // check intermediate percentage
      if (!user.intermediate_percentage) {
        setShowPopup(true);
        return;
      }
      ...
      if (!user.profile_completed) {
        window.location.href = `/user/profile?course=${slug}`;
        return;
      }

      // all good → go to confirmation page
      window.location.href = `/course-confirmation/${slug}`;
